    ...
    def __getitem__(self, idx):
        ...
        for fp in paths:
            try:
                ...
                # Use square dummy image if error occurs
...
class EfficientNetV2L_BiLSTM_CrossAttn_CBAM(nn.Module):
    def __init__(self, lstm_hidden=256, lstm_layers=1, dropout_rate=0.5, classifier_hidden=256):
        ...
        self.classifier = nn.Linear(classifier_hidden * 2, 16)  # 16 outputs (reshape to 4x4)
    def forward(self, x):
        # x: (B, T, C, H, W)
        B, T, C, H, W = x.size()
...
def progressive_train_model(model, total_epochs, lr, checkpoint_path, batch_size,
    ...
    # Set lower learning rate for backbone parameters
