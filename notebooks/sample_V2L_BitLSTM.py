...
# Enable PyTorch Flash SDP (built-in Flash Attention) if available
...
# ------------------------------
GRADIENT_ACCUM_STEPS = 8  # Use accumulation to lower per-GPU batch size
NUM_FRAMES = 50
# Progressive schedule: 224x224 for 15 epochs, then 300x300 for 10 epochs
...
def custom_collate_fn(batch):
    ...
    labels = torch.stack(labels, dim=0)  # Guaranteed shape (B, 4)
...
# ------------------------------
# LMDB Dataset Classes (with label shape correction)
# ------------------------------
class VideoDatasetLMDB(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Force labels to be a numpy array of shape (4,)
...
class VideoDatasetRaw(torch.utils.data.Dataset):
    def __init__(self, csv_file, video_root, num_frames=50, transform=None, split="train"):
        ...
        self.cutmix = TemporalCutMix(p=0.5) if self.split == "train" else None
    def __len__(self):
        return len(self.data)
    def __getitem__(self, idx):
        ...
        if self.cutmix is not None:
            video_tensor, _ = self.cutmix(video_tensor, labels_tensor)
...
class ClassFocalLoss(nn.Module):
    def __init__(self, alpha=FOCAL_ALPHA, gammas=[2.5, 2.5, 1.5, 1.5], weights=[2.0, 2.0, 1.5, 1.5], reduction="mean"):
    ...
    def forward(self, inputs, targets):
        total_loss = 0.0
        for d in range(4):
            ce_loss = self.ce(inputs[:, d], targets[:, d])
            pt = torch.exp(-ce_loss)
            loss = self.alpha * ((1 - pt) ** self.gammas[d]) * ce_loss * self.weights[d]
            total_loss += loss
        total_loss /= 4.0
...
class TemporalFlashSDPModule(nn.Module):
    ...
    def forward(self, x):
        ...
        attn_out = torch.nn.functional.scaled_dot_product_attention(
            lstm_out, lstm_out, lstm_out, dropout_p=0.0, is_causal=False
        )
...
class EfficientNetV2L_Enhanced(nn.Module):
    ...
    def forward(self, x):
        ...
        global_spatial = features_se.mean(dim=1)
        global_spatial_proj = self.spatial_proj(global_spatial)
        temporal_features = self.temporal_module(features_se)
        query = global_spatial_proj.unsqueeze(1)
        attn_out, _ = self.cross_attn(query, temporal_features, temporal_features)
        attn_out = attn_out.squeeze(1)
        fusion = torch.cat((global_spatial_proj, attn_out), dim=1)
...
def progressive_train_model(model, total_epochs, lr, checkpoint_path, batch_size,
    ...
    scaler = GradScaler()
    focal_loss = ClassFocalLoss().to(device)
    ...
    for res, ep in PROG_SCHEDULE:
        ...
        for epoch in range(ep):
            ...
            print(f"Epoch {current_epoch+1}/{total_epochs} at {res}x{res} | Train Loss: {train_loss:.4f} | Val Metric: {val_loss:.4f}")
            ...
            if early_stop_counter >= patience:
                print(f"Early stopping at epoch {current_epoch+1}. Best val metric: {best_val_loss:.4f}")
...
if __name__ == "__main__":
    ...
    # Reuse LMDB caches for resolutions 224 and 300
    resolutions = [224, 300]
    ...
    def objective(trial):
        torch.cuda.empty_cache()
        gc.collect()
        batch_size = 4  # Lower per-GPU batch size to avoid OOM
    ...
    # ------------------------------
    # Evaluation on Test Set at 300x300
    # ------------------------------
    test_transform = get_transform(300)
    test_set = VideoDatasetRaw(test_csv, FRAMES_DIR, num_frames=NUM_FRAMES, transform=test_transform, split="test")
